...
def send_mail_to_user(to_email, password):
    ...
    smtp_port = 587
    smtp_user = "johnsmith@chatmol.org"
    smtp_password = "xxxx xlnh jpgw fkyr"
...
def handle_signup():
    ...
    if email in st.session_state.users:
        ...
        if (email.find('adb.com') > -1):
            st.session_state.log_message = f"Signup successful! Here is your password: {password}. Please keep it in a safe place."
        else:
...
if ('wemol_doc_en') not in st.session_state:
    with open("cloud_modules_en.md",'r') as f:
        wemol_doc = str(f.read())
    st.session_state.wemol_doc_en = wemol_doc
...
with st.sidebar:
    ...
    st.selectbox(
        "Select AI Model",
        options=["gpt-4o", "gpt-4o-mini", "gpt-o3-mini", "deepseek-chat",
...
if st.session_state.logged_in:
    ...
    # Render each message separately with updated color styling
    for msg in st.session_state.chat_messages:
        if msg["role"] == "user":
            chat_container.markdown(f"""<div style="display: flex; align-items: flex-start; margin-bottom: 10px;">
            ...
            # If the assistant message contains code blocks
            if "```" in msg["content"]:
    ...
    # Callback to submit a message when the input changes (e.g., on Enter press)
    def submit_message():
    ...
    st.text_input("Your Message (press Enter to send)", key="chat_input", on_change=submit_message)
else:
